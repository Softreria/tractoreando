  ...
  try {
    ...
    for (const credentials of credentialsList) {
      try {
        ...
        const loginResponse = await axios.post('http://localhost:3000/api/auth/login', credentials);
    ...
    const maintenanceResponse = await axios.post('http://localhost:3000/api/maintenance', maintenanceData, {
      headers: {
        'Authorization': `Bearer ${token}`,
    ...
    const verifyResponse = await axios.get(`http://localhost:3000/api/maintenance/${maintenanceResponse.data.id}`, {
      headers: {
        'Authorization': `Bearer ${token}`
      }
    });
