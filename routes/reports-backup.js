...
router.get('/dashboard', [
  ...
  try {
    ...
    // Consultas paralelas para obtener estadísticas con manejo de errores individual
    let vehicleStats, maintenanceStats, fuelStats, recentMaintenances, upcomingMaintenances, alerts, userStats, companyStats, branchStats, maintenanceChart, vehicleDistribution;
    
    try {
      vehicleStats = await getVehicleStats(baseQuery);
    } catch (error) {
      console.error('Error en getVehicleStats:', error);
      vehicleStats = { total: 0, active: 0, inactive: 0, inMaintenance: 0 };
    }
    
    try {
      maintenanceStats = await getMaintenanceStats(baseQuery, dateFrom, dateTo);
    } catch (error) {
      console.error('Error en getMaintenanceStats:', error);
      maintenanceStats = { total: 0, completed: 0, pending: 0, overdue: 0 };
    }
    
    try {
      fuelStats = await getFuelStats(baseQuery, dateFrom, dateTo);
    } catch (error) {
      console.error('Error en getFuelStats:', error);
      fuelStats = { totalLiters: 0, totalCost: 0, averageConsumption: 0 };
    }
    
    try {
      recentMaintenances = await getRecentMaintenances(baseQuery, 5);
    } catch (error) {
    ...
    try {
      upcomingMaintenances = await getUpcomingMaintenances(baseQuery, 10);
    } catch (error) {
    ...
    try {
      alerts = await getMaintenanceAlerts(baseQuery);
    } catch (error) {
    ...
    try {
      userStats = await getUserStats(baseQuery, req.user);
    } catch (error) {
    ...
    try {
      companyStats = await getCompanyStats(req.user);
    } catch (error) {
    ...
    try {
      branchStats = await getBranchStats(baseQuery, req.user);
    } catch (error) {
    ...
    try {
      maintenanceChart = await getMaintenanceChart(baseQuery);
    } catch (error) {
    ...
    try {
      vehicleDistribution = await getVehicleDistribution(baseQuery);
    } catch (error) {
